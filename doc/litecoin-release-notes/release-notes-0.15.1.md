Boscocoin Core version *0.15.1* is now available from:

  <https://download.boscocoin.org/boscocoin-0.15.1/>
...
A number of changes to the way Boscocoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 dollars per kB (0.00001 BOSC). This is to prepare for dropping the minimum transaction fee to 0.1 dollars per kB (0.0001 BOSC) in the next release.
